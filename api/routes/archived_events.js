...
router.get('/byurl/:url', token.checkToken(false), async (req, res) => {
	try {
		const object = await ArchivedEvent.findOne({ url: new RegExp(`^${escapeStringRegexp(req.params.url.trim())}$`, 'i') });
		if (!object)
...
router.get('/name/:name', token.checkToken(false), async (req, res) => {
	try {
		const events = await ArchivedEvent.find({ name: new RegExp(`^${escapeStringRegexp(req.params.name.trim())}$`, 'i') });
		if (events.length === 0)

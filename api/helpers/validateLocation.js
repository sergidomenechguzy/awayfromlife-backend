      ...
      if (
        !(
          ...
          (typeof data.status === 'string' &&
            (data.status === 'opened' || data.status === 'closed'))
        )
      ) {
        return resolve(
          "Attribute 'status' can be left out or has to be either 'opened' or 'closed' as a string."
        );
      }
  ...
  return async (req, res, next) => {
    try {
      ...
      for (const current of data.list) {
        const response = await validateLocation(current, type, { urlList });
        if (typeof response === 'string') {
          return res.status(400).json({ message: response, token: res.locals.token });
        }
